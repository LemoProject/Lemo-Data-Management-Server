...
public class QFrequentPathsFortenbacher extends Question {
	...
	public ResultListUserPathGraph compute(
		...
		List<int[]> list = generateList(courses, users, types, minLength, maxLength, startTime, endTime,
				session, gender, learningObjects);
		
		int absoluteSupport = (int)(list.size() * minSup);
		
		Apriori.apriori(list, absoluteSupport);
	...
	public List<int[]> generateList(final List<Long> courses, List<Long> users,
		...
		for(ILog log : list)
		{
			if(!hasTypes || types.contains(log.getLearning().getLOType()))
			{
				...
				if(id != log.getUser().getId())
				{
					...
					{
						int[] userPath = new int[path.size()];
				...
				path.add(log.getLearning().getId());
			}
		}		
