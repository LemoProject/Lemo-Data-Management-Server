       	       			...
       	       			if(!numericUserId)
       	       			{
	       	    			long id = -1;
	       	       			...
	       	       			if(id == -1 )
	       	       			{
	       	       				id = largestId + 1;
	       	       				largestId = id;
	       	       				idMapping.put(loadedItem2.getUserid(), new IDMappingMining(Long.valueOf(platform.getPrefix() + "" + id), loadedItem2.getUserid(), platform.getId()));
	       	       				insert.setUser(Long.valueOf(platform.getPrefix() + "" + id),userMining, oldUserMining);
	       	       			}
       	       			}
    			...
    			if(!numericUserId)
    			{
	    			long id = -1;
	       			...
	       			if(id == -1 )
	       			{
	       				id = largestId + 1;
	       				largestId = id;
	       				idMapping.put(loadedItem.getUserid(), new IDMappingMining(Long.valueOf(platform.getPrefix() + "" + id), loadedItem.getUserid(), platform.getId()));
	       				insert.setUser(Long.valueOf(platform.getPrefix() + "" + id),userMining, oldUserMining);
	       			}
    			}
    			...
    			if((loadedItem.getAction().equals("view forum") || loadedItem.getAction().equals("subscribe")) && loadedItem.getInfo().matches("[0-9]+")){
    				...
    				if((loadedItem.getAction().equals("add discussion") || loadedItem.getAction().equals("view discussion")) && loadedItem.getInfo().matches("[0-9]+")){
   			    		if(forumDis.get(Long.valueOf(loadedItem.getId())) != null)
    			    			insert.setForum(Long.valueOf(platform.getPrefix() + "" + forumDis.get(Long.valueOf(loadedItem.getId())).getForum()), forumMining, oldForumMining);
    				}
    			}
    			insert.setCourse(Long.valueOf(platform.getPrefix() + "" + loadedItem.getCourse()), courseMining, oldCourseMining);
    			insert.setAction(loadedItem.getAction());
    ...
    public Map<Long, UserMining> generateUserMining(){
    	...
    	for (User_LMS loadedItem : userLms) 
    	{
    		...
    		if(!numericUserId)
    		{
				long id = largestId + 1;
	   			largestId = id;
	   			idMapping.put(loadedItem.getId(), new IDMappingMining(Long.valueOf(platform.getPrefix() + "" + id), loadedItem.getId(), platform.getId()));
