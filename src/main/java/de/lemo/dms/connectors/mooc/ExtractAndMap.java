...
public abstract class ExtractAndMap {
	...
	protected Map<Long, UserAssessment> assessmentUserMining;
	...
	protected Map<Long, UserAssessment> oldAssessmentUserMining;
	...
	public long getMiningInitial() {
		...
		t = this.dbHandler.performQuery(session, EQueryType.HQL, "from TaskUser x"
				//+ " where x.platform=" + this.connector.getPlatformId() 
				+ " order by x.id asc");
		this.oldAssessmentUserMining = new HashMap<Long, UserAssessment>();
		for (int i = 0; i < t.size(); i++) {
			this.oldAssessmentUserMining.put(((UserAssessment) (t.get(i))).getId(), (UserAssessment) t.get(i));
		}
	 ...
	 **/
	abstract Map<Long, UserAssessment> generateAssessmentUserMining();
	 ...
	 **/
	abstract Map<Long, AccessLog> generateLearningLogMining();
