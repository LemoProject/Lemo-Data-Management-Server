...
//read & save LMS DB in steps of 2 days
	...
	public long getMiningInitial(String platformName){
		...
		configMiningTimestamp = (List<Timestamp>) dbHandler.performQuery(session, EQueryType.HQL, "select max(lastmodified) from ConfigMining x where x.platform="+ platform.getId() +" order by x.id asc");//mining_session.createQuery("select max(lastmodified) from ConfigMining x order by x.id asc").list();
		if(configMiningTimestamp.get(0) == null){
